...
app.post("/cautaCarte", function (req, res) {
    var form = new formidable.IncomingForm();
    form.parse(req, function (err, fields, files) {
        var sql = 'select ISBN, Titlu, Nume, Prenume, numeEditură, Cameră, numeLimbă, nrPagini, numeGen, numeCategorie ' +
...
app.get("/views/statistica.ejs", function (req, res) {
    con.query('SELECT count(*) nr FROM proiectbd.instanțăcarte;', function(err, result, fields) {
        if(err) throw err;
